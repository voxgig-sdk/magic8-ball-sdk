...
module Magic8BallUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Magic8BallResponse.new({})
    ...
    if fetch_err
      ...
      response = Magic8BallResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Magic8BallResponse.new(fetched)
    else
