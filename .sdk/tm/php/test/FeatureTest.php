...
class FtCtrl extends Magic8BallControl
{
...
class FtClient
{
    ...
    private Magic8BallUtility $utility;

    public function __construct(Magic8BallUtility $utility, array $options)
    {
    ...
    public function get_utility(): Magic8BallUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Magic8BallContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new Magic8BallUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (Magic8BallContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Magic8BallConfig::make_feature($name);
    ...
    private static function buildUrl(Magic8BallSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Magic8BallError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Magic8BallResponse($fetched);
        }
    ...
    private function fail(Magic8BallContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Magic8BallContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
