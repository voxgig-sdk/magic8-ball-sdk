...
class Magic8BallRetryFeature extends Magic8BallBaseFeature
{
    ...
    public function init(Magic8BallContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Magic8BallContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Magic8BallContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Magic8BallContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
