...
## Install
```bash
go get github.com/voxgig-sdk/magic8-ball-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/magic8-ball-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/magic8-ball-sdk/go=../magic8-ball-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *Magic8BallSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### Magic8BallSDK methods
...
All entities implement the `Magic8BallEntity` interface.
...
Create an instance: `biased := client.Biased(nil)`
...
```go
biased, err := client.Biased(nil).Load(map[string]any{"question": "question"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(biased) // the loaded record
```
...
```
github.com/voxgig-sdk/magic8-ball-sdk/go/
├── magic8-ball.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/magic8-ball-sdk/go`) re-exports everything needed
