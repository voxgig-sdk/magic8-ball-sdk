...
func TestCategoryFortuneEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		categoryFortuneRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.category_fortune")))
...
func category_fortuneBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID"])
