...
func TestRandomFortuneEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		randomFortuneRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.random_fortune")))
...
func random_fortuneBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MAGIC8_BALL_TEST_RANDOM_FORTUNE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAGIC8_BALL_TEST_RANDOM_FORTUNE_ENTID"])
