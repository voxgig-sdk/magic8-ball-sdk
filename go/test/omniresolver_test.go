...
import (
	...
	omni "github.com/voxgig-sdk/magic8-ball-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Magic8BallError); is {
				out := map[string]any{
					"name":    "Magic8BallError",
...
func omniCtx(arg any, client *sdk.Magic8BallSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Magic8BallSDK); is {
			client = live
