...
// Magic8Ball SDK CategoryFortune entity
...
class CategoryFortuneEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CategoryFortuneEntity($this->_client, $opts);
    }

    /**
     * @param CategoryFortune|array $args CategoryFortune data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = Magic8BallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CategoryFortune|array The current CategoryFortune data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CategoryFortune fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = Magic8BallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CategoryFortune fields).
     */
    ...
    /**
     * Load a single CategoryFortune.
     *
     * @param CategoryFortuneLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CategoryFortuneLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CategoryFortune|array The loaded CategoryFortune as an assoc-array at the
     *   SDK boundary; throws Magic8BallError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = Magic8BallHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
