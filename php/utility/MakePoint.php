...
// Magic8Ball SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Magic8BallMakePoint
{
    public static function call(Magic8BallContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Magic8BallError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Magic8BallHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Magic8BallHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
