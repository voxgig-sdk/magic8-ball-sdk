...
class Magic8BallSDK
{
            ...
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    ...
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!Magic8BallFeatures::has_feature($fname)) {
    ...
    {
        return Magic8BallUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = Magic8BallHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = Magic8BallHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = Magic8BallHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new Magic8BallError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = Magic8BallHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new Magic8BallError("graphql_error",
                "Magic8BallSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
