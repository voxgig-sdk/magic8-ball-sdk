...
class RandomFortuneEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_RANDOM_FORTUNE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function random_fortune_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MAGIC8_BALL_TEST_RANDOM_FORTUNE_ENTID");
