...
class CategoryFortuneEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Magic8BallSDK::test(null, null);
        $ent = $testsdk->CategoryFortune(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = category_fortune_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "category_fortune." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $category_fortune_ref01_data = null;
        if (count($category_fortune_ref01_data_raw) > 0) {
...
function category_fortune_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/category_fortune/CategoryFortuneTestData.json';
    ...
    $entid_env_raw = getenv("MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID");
