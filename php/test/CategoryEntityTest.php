...
class CategoryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = Magic8BallSDK::test($seed, null);
        $seen = iterator_to_array($base->Category(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Category(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = category_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "category." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_CATEGORY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $category_ref01_data = null;
        if (count($category_ref01_data_raw) > 0) {
...
function category_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/category/CategoryTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["category01", "category02", "category03"] as $k) {
    ...
    $entid_env_raw = getenv("MAGIC8_BALL_TEST_CATEGORY_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MAGIC8_BALL_TEST_CATEGORY_ENTID"]);
