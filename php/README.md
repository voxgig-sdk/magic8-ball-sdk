...
The PHP SDK for the Magic8Ball API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Biased()` — with named operations (`list`/`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $category));
```
...
```php
$client = Magic8BallSDK::test($testopts, $sdkopts);
```
...
```
php/
├── magic8ball_sdk.php          -- Main SDK class
...
The main class (`magic8ball_sdk.php`) exports the SDK class
