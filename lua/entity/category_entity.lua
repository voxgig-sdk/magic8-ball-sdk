...
CategoryEntity.__index = CategoryEntity


function CategoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CategoryEntity)
  self._name = "category"
  self._client = client
...
function CategoryEntity:get_name()
  return self._name
end


function CategoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CategoryEntity.new(self._client, opts)
end
...
function CategoryEntity:mark_deleted()
  self._deleted = true
end


function CategoryEntity:deleted()
  return true == self._deleted
end


function CategoryEntity:data_set(args)
  if args ~= nil then
...
function CategoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CategoryEntity:match_set(args)
  if args ~= nil then
...
function CategoryEntity:match_get()
...
function CategoryEntity:stream(action, args, callopts)
...
---@param reqmatch CategoryListMatch
---@param ctrl? table
---@return Category[]
---@return string? err
function CategoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CategoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
