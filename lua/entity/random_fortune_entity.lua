...
local RandomFortuneEntity = {}
RandomFortuneEntity.__index = RandomFortuneEntity


function RandomFortuneEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RandomFortuneEntity)
  self._name = "random_fortune"
  self._client = client
...
function RandomFortuneEntity:get_name()
  return self._name
end


function RandomFortuneEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RandomFortuneEntity.new(self._client, opts)
end
...
function RandomFortuneEntity:mark_deleted()
  self._deleted = true
end


function RandomFortuneEntity:deleted()
  return true == self._deleted
end


function RandomFortuneEntity:data_set(args)
  if args ~= nil then
...
function RandomFortuneEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RandomFortuneEntity:match_set(args)
  if args ~= nil then
...
function RandomFortuneEntity:match_get()
...
function RandomFortuneEntity:stream(action, args, callopts)
...
function RandomFortuneEntity:_run_op(ctx, post_done)
  local utility = self._utility
