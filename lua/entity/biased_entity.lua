...
local BiasedEntity = {}
BiasedEntity.__index = BiasedEntity


function BiasedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BiasedEntity)
  self._name = "biased"
  self._client = client
...
function BiasedEntity:get_name()
  return self._name
end


function BiasedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BiasedEntity.new(self._client, opts)
end
...
function BiasedEntity:mark_deleted()
  self._deleted = true
end


function BiasedEntity:deleted()
  return true == self._deleted
end


function BiasedEntity:data_set(args)
  if args ~= nil then
...
function BiasedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BiasedEntity:match_set(args)
  if args ~= nil then
...
function BiasedEntity:match_get()
...
function BiasedEntity:stream(action, args, callopts)
...
function BiasedEntity:_run_op(ctx, post_done)
  local utility = self._utility
