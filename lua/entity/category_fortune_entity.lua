...
local CategoryFortuneEntity = {}
CategoryFortuneEntity.__index = CategoryFortuneEntity


function CategoryFortuneEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CategoryFortuneEntity)
  self._name = "category_fortune"
  self._client = client
...
function CategoryFortuneEntity:get_name()
  return self._name
end


function CategoryFortuneEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CategoryFortuneEntity.new(self._client, opts)
end
...
function CategoryFortuneEntity:mark_deleted()
  self._deleted = true
end


function CategoryFortuneEntity:deleted()
  return true == self._deleted
end


function CategoryFortuneEntity:data_set(args)
  if args ~= nil then
...
function CategoryFortuneEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CategoryFortuneEntity:match_set(args)
  if args ~= nil then
...
function CategoryFortuneEntity:match_get()
...
function CategoryFortuneEntity:stream(action, args, callopts)
...
function CategoryFortuneEntity:_run_op(ctx, post_done)
  local utility = self._utility
