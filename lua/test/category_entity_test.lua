...
describe("CategoryEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_CATEGORY_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local category_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.category")))
    local category_ref01_data = nil
    if #category_ref01_data_raw > 0 then
      category_ref01_data = helpers.to_map(category_ref01_data_raw[1][2])
    end
...
function category_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MAGIC8_BALL_TEST_CATEGORY_ENTID")
