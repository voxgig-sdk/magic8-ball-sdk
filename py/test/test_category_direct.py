...
class TestCategoryDirect:

    def test_should_direct_list_category(self):
        setup = _category_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-category", "live" if setup["live"] else "unit")
        if _skip:
...
def _category_direct_setup(mockres):
