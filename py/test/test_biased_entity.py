...
class TestBiasedEntity:

    def test_should_create_instance(self):
        testsdk = Magic8BallSDK.test(None, None)
        ent = testsdk.Biased(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _biased_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "biased." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAGIC8_BALL_TEST_BIASED_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        biased_ref01_ent = client.Biased(None)
        biased_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.biased"), "biased_ref01"))

        biased_ref01_data = helpers.to_map(runner.entity_data(biased_ref01_ent.create(biased_ref01_data, None)))
...
def _biased_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/biased/BiasedTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAGIC8_BALL_TEST_BIASED_ENTID"))
