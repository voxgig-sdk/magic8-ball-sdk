...
class TestCategoryFortuneDirect:

    def test_should_direct_load_category_fortune(self):
        setup = _category_fortune_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-category_fortune", "live" if setup["live"] else "unit")
        if _skip:
...
def _category_fortune_direct_setup(mockres):
