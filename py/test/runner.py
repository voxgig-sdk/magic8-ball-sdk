...
class Magic8BallTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Magic8BallTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Magic8BallTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Magic8BallTestRunner.getenv("MAGIC8_BALL_TEST_LIVE")
        override = Magic8BallTestRunner.getenv("MAGIC8_BALL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Magic8BallTestRunner.getenv(key)
        ...
        explain = Magic8BallTestRunner.getenv("MAGIC8_BALL_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if Magic8BallTestRunner._test_control is not None:
            return Magic8BallTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Magic8BallTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Magic8BallTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Magic8BallTestRunner.load_test_control()
...
def load_env_local():
    Magic8BallTestRunner.load_env_local()


def env_override(m):
    return Magic8BallTestRunner.env_override(m)


def entity_data(v):
    return Magic8BallTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Magic8BallTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Magic8BallTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Magic8BallTestRunner.load_test_control()


def live_client_options():
    return Magic8BallTestRunner.live_client_options()


def live_delay_ms():
    return Magic8BallTestRunner.live_delay_ms()
