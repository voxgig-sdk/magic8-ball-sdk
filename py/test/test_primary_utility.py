...
from magic8ball_sdk.core.spec import Magic8BallSpec
from magic8ball_sdk.core.result import Magic8BallResult
from magic8ball_sdk.core.response import Magic8BallResponse
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return Magic8BallError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = Magic8BallSDK.test(None, None)
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(Magic8BallBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = Magic8BallSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = Magic8BallSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = Magic8BallSDK.test(None, setup_opts)
