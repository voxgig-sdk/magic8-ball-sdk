...
class TestCategoryEntity:
    ...
    def test_should_stream(self):
        ...
        base = Magic8BallSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAGIC8_BALL_TEST_CATEGORY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        category_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.category")))
        category_ref01_data = None
        if len(category_ref01_data_raw) > 0:
...
def _category_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAGIC8_BALL_TEST_CATEGORY_ENTID"))
