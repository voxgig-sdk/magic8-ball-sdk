...
from magic8ball_sdk.magic8ball_types import (
    CategoryFortune,
...
class CategoryFortuneEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CategoryFortuneEntity(self._client, opts)
    ...
    def data_get(self) -> CategoryFortune:
    ...
    def match_get(self) -> CategoryFortune:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CategoryFortune:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CategoryFortune().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
