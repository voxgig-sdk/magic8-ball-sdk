...
`client.Biased()` — each with a small set of operations (`list`, `load`, `create`)
...
| Method | Returns | Description |
| --- | --- | --- |
| `Magic8BallSDK.test(testopts?, sdkopts?)` | `Magic8BallSDK` | Create a test-mode client. |
...
```ts
const biased = await client.Biased().load({ question: 'question' })
```
...
magic8-ball/
├── src/
│   ├── Magic8BallSDK.ts        # Main SDK class
