...
//     (\`new Magic8BallSDK(...)\` / \`Magic8BallSDK.test(...)\` become
//     \`Magic8BallSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new Magic8BallSDK(...)\` and any
// \`Magic8BallSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Magic8BallSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Magic8BallSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
