...
describe('CategoryFortuneDirect', async () => {
  ...
  test('direct-load-category_fortune', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-category_fortune', setup.live)) return
    if (skipIfMissingIds(t, setup, ["category01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID']
