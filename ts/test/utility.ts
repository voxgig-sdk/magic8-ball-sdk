...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MAGIC8BALL_TEST_LIVE ||
    'TRUE' === process.env.MAGIC8BALL_TEST_OVERRIDE
  ) {
  ...
  m.MAGIC8BALL_TEST_EXPLAIN = process.env.MAGIC8BALL_TEST_EXPLAIN || m.MAGIC8BALL_TEST_EXPLAIN
