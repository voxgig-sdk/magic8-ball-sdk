...
(0, node_test_1.describe)('CategoryFortuneEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'category_fortune.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['category_fortune01', 'category_fortune02', 'category_fortune03', 'api01', 'api02', 'api03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID'];
