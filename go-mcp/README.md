...
[MCP](https://modelcontextprotocol.io) server exposing the Magic8Ball SDK as
two agent tools — `magic8-ball_list` and `magic8-ball_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/magic8-ball-mcp)
make build

# 2. Provide credentials via the environment
export MAGIC8_BALL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user magic8-ball \
  -- /absolute/path/to/magic8-ball-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./magic8-ball-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// magic8-ball_list: first page of records
{ "entity": "category" }
{ "entity": "category", "query": { } }

// magic8-ball_load: one record by id
{ "entity": "biased", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/magic8-ball-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user magic8-ball \
     -- "$PWD"/dist/*/magic8-ball-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `magic8-ball_list` and `magic8-ball_load` tools now appear
   in new sessions. Ask the agent to *"list category using magic8-ball"*
   and it calls `magic8-ball_list` with `{"entity":"category"}`.
...
```sh
export MAGIC8_BALL_APIKEY=sk_live_xxx            # API key
export MAGIC8_BALL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `magic8-ball_list` tool
...
### Call the `magic8-ball_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `magic8-ball_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `magic8-ball_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MAGIC8_BALL_APIKEY` | API key sent with every request. |
| `MAGIC8_BALL_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

biased | category | category_fortune | random_fortune

### Smoke test via HTTP (raw JSON-RPC)

```sh
./magic8-ball-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"magic8-ball_load","arguments":{"entity":"biased","query":{"id":1}}}}'
```
