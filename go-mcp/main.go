// Command Magic8Ball-mcp is an MCP server exposing the Magic8Ball
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Magic8Ball_list  — list records of an entity
//	Magic8Ball_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MAGIC8_BALL_APIKEY carries the API key and
	// MAGIC8_BALL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MAGIC8_BALL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MAGIC8_BALL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMagic8BallSDK(opts)
