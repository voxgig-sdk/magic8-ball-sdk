...
require_relative 'Magic8Ball_types'
...
class Magic8BallSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = Magic8BallHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = Magic8BallHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, Magic8BallFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    Magic8BallUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = Magic8BallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = Magic8BallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = Magic8BallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => Magic8BallError.new(
        "#{op}_allow",
        "Magic8BallSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = Magic8BallHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = Magic8BallError.new(
        "graphql_error", "Magic8BallSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
