...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = Magic8BallOmni.make_runner(TEST_JSON_FILE, Magic8BallSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    Magic8BallError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : Magic8BallOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = Magic8BallOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
