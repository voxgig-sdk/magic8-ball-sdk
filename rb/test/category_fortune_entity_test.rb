...
class CategoryFortuneEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = category_fortune_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "category_fortune." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    category_fortune_ref01_data = nil
    if category_fortune_ref01_data_raw.length > 0
...
def category_fortune_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "category_fortune", "CategoryFortuneTestData.json")
  ...
  entid_env_raw = ENV["MAGIC8_BALL_TEST_CATEGORY_FORTUNE_ENTID"]
  ...
  if env["MAGIC8_BALL_TEST_LIVE"] == "TRUE"
    ...
    client = Magic8BallSDK.new(Helpers.to_map(merged_opts))
  end
