...
module Magic8BallOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if Magic8BallOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = Magic8BallOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| Magic8BallOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || Magic8BallOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = Magic8BallSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = Magic8BallError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { Magic8BallOmni.errify(err) }
