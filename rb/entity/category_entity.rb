...
class CategoryEntity
  ...
  def make
    opts = @_entopts.dup
    CategoryEntity.new(@_client, opts)
  end
  ...
  # @return [Category, Hash] the current Category data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Category fields)
  def match_get
  ...
  #
  # @param reqmatch [CategoryListMatch, Hash, nil] match filter (any subset of
  #   Category fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Category>, Array] the matching Category items; raises Magic8BallError on failure
